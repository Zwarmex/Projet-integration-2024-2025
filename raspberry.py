...
def distribuer_friandise():
    ...
    message = {
        "event": "bouton_friandise_appuyé, récompense"
        }
    if rewards_day < max_rewards:
        print("Bouton friandise détecté, distribution en cours...")
        moteur_friandise()
        rewards_day += 1
    
    else:
        print("Bouton friandise détecté, mais max journalier déjà atteint")
        message = {
        "event": "bouton_friandise_appuyé, maximum déà atteint"
        }
        ...
        try:
            ...
            while True:
                ...
                if time.ticks_diff(temps_actuel, dernier_verif_temps) >= intervalle_verification:
                    ...
                    if temps_actuel//(1000*60*60*24) != dernier_verif_temps//(1000*60*60*24):
                        global rewards_day
                        rewards_day = 0
